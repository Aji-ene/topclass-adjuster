...
async function callGrok({ model, prompt, textFiles, imageFiles, temperature, max_tokens, metadata }) {
  ...
  const messages = [
    { role: 'system', content: 'You are Grok — expert insurance analyst and adjuster with deep knowledge of claims processing and risk assessment.' }
  ];
  ...
  if (metadata.structuredHeadlines && metadata.structuredHeadlines.length > 0) {
    metadata.structuredHeadlines.forEach(h => {
      ...
      if (mainHeadline.includes('THE INSURED')) {
        ...
        focus += `\n\n${h.main}:\nCarefully review the Policy Document and any Endorsements provided. First, in sub points/headers list all applicable Memos, Clauses, Warranties, Conditions, and Exclusions that are RELEVANT to this specific claim with a relevant write up following each in essay format. Thereafter, also in sub points/headers list those that are NOT relevant to this claim and do not write any write up for these ones.`;
